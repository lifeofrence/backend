@extends('layouts.admin', ['title' => 'Upload Key Material'])
...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="card p-8">
            <form action="{{ route('admin.key-materials.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="category" required class="w-full rounded-lg px-4 py-2.5 focus:outline-none">
                            <option value="quarterly_results" {{ old('category') == 'quarterly_results' ? 'selected' : '' }}>Quarterly Results</option>
                            <option value="annual_materials" {{ old('category') == 'annual_materials' ? 'selected' : '' }}>Annual Materials</option>
                            <option value="corporate_responsibility_disclosures" {{ old('category') == 'corporate_responsibility_disclosures' ? 'selected' : '' }}>Corporate Responsibility Disclosures</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" required class="w-full rounded-lg px-4 py-2.5 focus:outline-none"
                        placeholder="e.g. 2025 Annual Report" value="{{ old('title') }}">
                </div>
