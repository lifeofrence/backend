@extends('layouts.admin', ['title' => 'Edit Press Release'])
...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="card p-8">
            <form action="{{ route('admin.press-releases.update', $press_release) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" required class="w-full rounded-lg px-4 py-2.5 focus:outline-none"
                        value="{{ old('title', $press_release->title) }}">
                </div>

                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-400">Date</label>
                        <input type="date" name="date" required class="w-full rounded-lg px-4 py-2.5 focus:outline-none" value="{{ old('date', \Carbon\Carbon::parse($press_release->date)->format('Y-m-d')) }}">
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-400">Slug</label>
                        <input type="text" name="slug" class="w-full rounded-lg px-4 py-2.5 focus:outline-none"
                            value="{{ old('slug', $press_release->slug) }}">
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="text-sm font-medium text-zinc-400">Content (Optional)</label>
                    <textarea name="content" rows="6" class="w-full rounded-lg px-4 py-2.5 focus:outline-none">{{ old('content', $press_release->content) }}</textarea>
                </div>

                <div class="space-y-2">
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $press_release->is_active) ? 'checked' : '' }}
