@extends('layouts.admin', ['title' => 'Press Releases Management'])

@section('content')
    <div class="flex justify-between items-center mb-8">
        <h3 class="text-2xl font-semibold">Press Releases</h3>
        @unlessrole('Viewer')
    ...
    <div class="card overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-white/5">
                @forelse($press_releases as $release)
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        <td class="px-6 py-4 text-white text-sm font-medium">{{ \Carbon\Carbon::parse($release->date)->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-zinc-400 text-sm">{{ $release->title }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right space-x-3">
                            @unlessrole('Viewer')
                            <a href="{{ route('admin.press-releases.edit', $release) }}"
                                class="text-[#DC833D] hover:underline text-sm font-medium">Edit</a>
                            <form action="{{ route('admin.press-releases.destroy', $release) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-zinc-500 italic">No press releases found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
